...
public class TransferRestResourceTest {
    ...
    public void transferAmountWithValidAccountsAndAmount() {
        if (checkApplicationRunning()) {
            ...
            JsonPath response = given().basePath("transactions/accounts/").
                    ...
                    .body(requestBody)
                    .when().put("/{sourceAccountId}/{targetAccountId}/transfers")
    ...
    public void withdrawalAmountWithInValidAccounts() {
        if (checkApplicationRunning()) {
            ...
            String message = response.getString("message");
            String result = response.get("result");
            assertEquals("Account not found for accountId: 0", message);
    ...
    public void depositAmountWithInValidCurrencyCode() {
        if (checkApplicationRunning()) {
            ...
            String message = response.getString("message");
            String result = response.get("result");
            assertEquals("Unknown currency code: abca", message);
            assertEquals("Failure", result);
        }
    }

    @Test
    @DisplayName("withdrawal should fail when requested with bigger amount than available amount in account")
    public void withdrawalWillFailWhenRequestWithBiggerAmountThanAvailableAmount() {
        if (checkApplicationRunning()) {
            String requestBody = "{\"amount\":2200.00,\"currencyCode\":\"USD\"}";
            JsonPath response = given().basePath("transactions/accounts/").
                    pathParam("sourceAccountId", 70001)
                    .body(requestBody)
                    .when().put("/{sourceAccountId}/withdrawals")
            ...
            String message = response.getString("message");
            String result = response.get("result");
            assertEquals("Not enough funds for withdrawal", message);
            assertEquals("Failure", result);
        }
     ...
     * the integration  test will be skipped if the application is not available to run integration tests
    ...
    private boolean checkApplicationRunning() {
        ...
        try (Socket socket = new Socket()) {
            socket.connect(new InetSocketAddress("127.0.0.1", 8089), 2000);
            return true;
        } catch (IOException e) {
